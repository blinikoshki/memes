...
public class SessionReportingCoordinator implements CrashlyticsLifecycleEvents {
    ...
    public void persistUserId(String str) {
        ...
        if (userId == null) {
            Logger.getLogger().mo53729d("Could not persist user ID; no user ID available");
        } else {
    ...
    private void persistEvent(Throwable th, Thread thread, String str, String str2, long j, boolean z) {
        ...
        if (logString != null) {
            ...
            Logger.getLogger().mo53729d("No log data to include with this event.");
        }
    ...
    public boolean onReportSendComplete(Task<CrashlyticsReportWithSessionId> task) {
        if (task.isSuccessful()) {
            ...
            Logger logger = Logger.getLogger();
            logger.mo53729d("Crashlytics report successfully enqueued to DataTransport: " + result.getSessionId());
            this.reportPersistence.deleteFinalizedReport(result.getSessionId());
            return true;
        }
        Logger.getLogger().mo53730d("Crashlytics report could not be enqueued to DataTransport", task.getException());
        return false;
    }
