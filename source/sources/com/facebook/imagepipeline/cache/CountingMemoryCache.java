...
public class CountingMemoryCache<K, V> implements MemoryCache<K, V>, MemoryTrimmable {
    ...
    static class Entry<K, V> {
        ...
        /* renamed from: of */
        static <K, V> Entry<K, V> m129of(K k, CloseableReference<V> closeableReference, @Nullable EntryStateObserver<K> entryStateObserver) {
    ...
    public CloseableReference<V> cache(K k, CloseableReference<V> closeableReference, EntryStateObserver<K> entryStateObserver) {
        ...
        synchronized (this) {
            ...
            if (canCacheNewValue(closeableReference.get())) {
                Entry<K, V> of = Entry.m129of(k, closeableReference, entryStateObserver);
    ...
    public void probe(K k) {
        Preconditions.checkNotNull(k);
        synchronized (this) {
            Entry remove = this.mExclusiveEntries.remove(k);
            if (remove != null) {
    ...
    private synchronized CloseableReference<V> newClientReference(final Entry<K, V> entry) {
        ...
        return CloseableReference.m123of(entry.valueRef.get(), new ResourceReleaser<V>() {
