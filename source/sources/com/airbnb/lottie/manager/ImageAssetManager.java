...
public class ImageAssetManager {
    ...
    public ImageAssetManager(Drawable.Callback callback, String str, ImageAssetDelegate imageAssetDelegate, Map<String, LottieImageAsset> map) {
        ...
        if (!(callback instanceof View)) {
            Log.w(C0947L.TAG, "LottieDrawable must be inside of a view for images to work.");
    ...
    public Bitmap bitmapForId(String str) {
        ...
        if (!fileName.startsWith("data:") || fileName.indexOf("base64,") <= 0) {
            try {
                ...
                Log.w(C0947L.TAG, "Unable to open asset.", e);
                return null;
            }
        } else {
            try {
                ...
                Log.w(C0947L.TAG, "data URL did not have correct base64 format.", e2);
                return null;
            }
        }
    }
    ...
    public void recycleBitmaps() {
        synchronized (bitmapHashLock) {
            ...
            while (it.hasNext()) {
                Bitmap bitmap = ((LottieImageAsset) it.next().getValue()).getBitmap();
                if (bitmap != null) {
                    bitmap.recycle();
                }
                it.remove();
