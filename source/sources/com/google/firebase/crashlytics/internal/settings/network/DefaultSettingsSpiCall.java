...
public class DefaultSettingsSpiCall implements SettingsSpiCall {
    ...
    public JSONObject invoke(SettingsRequest settingsRequest, boolean z) {
        if (z) {
            try {
                ...
                logger2.mo53729d("Requesting settings from " + this.url);
                Logger logger3 = this.logger;
                logger3.mo53729d("Settings query params were: " + queryParamsFor);
                return handleResponse(applyHeadersTo.execute());
            } catch (IOException e) {
                this.logger.mo53732e("Settings request failed.", e);
                return null;
            }
        } else {
    ...
    public JSONObject handleResponse(HttpResponse httpResponse) {
        ...
        Logger logger2 = this.logger;
        logger2.mo53729d("Settings result was: " + code);
        if (requestWasSuccessful(code)) {
            return getJsonObjectFrom(httpResponse.body());
        }
        Logger logger3 = this.logger;
        logger3.mo53731e("Failed to retrieve settings from " + this.url);
        return null;
    }
    ...
    private JSONObject getJsonObjectFrom(String str) {
        try {
            ...
            Logger logger2 = this.logger;
            logger2.mo53730d("Failed to parse settings JSON from " + this.url, e);
            Logger logger3 = this.logger;
            logger3.mo53729d("Settings response " + str);
            return null;
        }
    }
